...
export function Hero() {
  return (
    <section
      id="hero"
      className="relative flex min-h-dvh items-center overflow-hidden bg-(--color-accent-strong)"
    >
      <Image
        src="/images/hero.svg"
        alt="Softly lit reception space styled with olive and ivory florals beneath a floral arch"
        fill
        priority
        sizes="100vw"
      ...
      />
      <div
        className="absolute inset-0 bg-gradient-to-t from-(--color-ink)/70 via-(--color-ink)/35 to-(--color-ink)/20"
        aria-hidden="true"
      />
      ...
      <div className="relative mx-auto flex w-full max-w-4xl flex-col items-start px-6 pt-24 lg:px-10">
        ...
        <a
          href="#contact"
          className="mt-10 inline-flex min-h-12 items-center justify-center rounded-full bg-(--color-ivory) px-8 text-sm font-medium tracking-wide text-(--color-accent-strong) transition-[background-color,transform] duration-150 ease-out hover:bg-white active:scale-[0.97]"
        >
          Inquire Now
        </a>
      </div>
